...
未接続なら https://hitocall.com/docs のクライアント別手順でMCP接続を追加し、OAuth認証が必要な場合のみ利用者に案内する。接続後はtools/listで利用できるツールと入力スキーマを確認する。get_setupが公開されていれば接続中の会社とnext_actionを確認する。公開されていなければ、list_campaigns、list_ordersのうち利用できる取得ツールで対象を確認する。どちらもなければ公開ツールと利用者の資料から対象IDを確認し、不明点だけ質問する。以降の操作手順は、そのツールが公開され、利用者から依頼された範囲でのみ適用する。権限のない会社設定・発注・支払い・録音閲覧を試行したり完了と報告したりせず、必要な作業と不足する権限を伝える。秘密キー・パスワードをチャットで求めない。
